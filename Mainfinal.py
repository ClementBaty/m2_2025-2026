...
def recherche_colonne(df):
    ...
    """
    # Noms prioritaires définis par l'équipe B
    noms_prioritaires = [
...
def filtrer_signal(input_signal, sampling_rate, type_filtre, ordre_filtre,
    ...
    """Applique d'un filtre sur un signal.
...
def Normalisation_signal(signal):
    """Normaliser le signal dansl'intervale [-1;1]
    """
...
def extraction_amp_freq(signal, fs):
    ...
    utilisation de la transformée de Fourier rapide (FFT) pour identifier
...
def connecter_signaux(win):
    """Connection de chaque widget de l'interface à sa fonction de traitement.
    """
