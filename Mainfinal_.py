...
from interface20 import MainApp

# Dictionnaire global de suivi de l'état du signal et des filtres
etat = {
    ...
    "meta": {               # dict       — paramètres du dernier filtre appliqué
        ...
        "fc2": None,        # fréquence de coupure haute (Hz)
...
def conversion_chaine_liste(value):
    ...
    Supporte les formats : liste Python, tuple, valeurs séparées par des virgules.

    Args:
        value (any): La valeur d'entrée à convertir (str, list, ndarray, etc.).

    Returns:
        np.ndarray: Tableau de réels (float), ou None si la conversion échoue.
    """
...
def recherche_colonne(df):
    ...
    Args:
        df (pd.DataFrame): Le DataFrame dans lequel chercher le signal.

    Returns:
        str: Nom de la colonne détectée, ou None si aucune n'est trouvée.
    """
    noms_prioritaires = [
...
def construire_axe_temps(df, n):
    ...
    Si une colonne temporelle est trouvée, calcule la fréquence
    d'échantillonnage (fs) automatiquement. Sinon, génère un axe synthétique.

    Args:
        df (pd.DataFrame): Le DataFrame contenant potentiellement le temps.
        n (int): Nombre de points du signal.

    Returns:
        np.ndarray: Axe temporel calculé ou généré en secondes.
    """
    ...
    # Aucune colonne temps trouvée : axe synthétique basé sur etat['fs']
...
def charger_signal(file_path):
    ...
    Détecte automatiquement la colonne du signal et construit l'axe temporel.
    Met à jour les clés globales 'signal_brut', 'temps' et 'fs' de 'etat'.

    Args:
        file_path (str): Chemin absolu ou relatif du fichier à charger.

    Raises:
        ValueError: Si le fichier est vide ou si le signal est indétectable.
    """
    ...
    if ext == ".csv":
        ...
        if col_signal is not None:
            # Cas d'une colonne contenant des listes encodées en texte
            signaux = []
        ...
        # Fallback : concaténation de toutes les colonnes numériques
...
def generer_signal_test(fs=1000.0, duree=2.0):
    """Génère un signal de test synthétique.

    Le signal généré comprend deux sinusoïdes (5 Hz et 50 Hz) et un bruit
    gaussien d'amplitude 0.3. Utilisé pour valider le comportement des filtres.

    Args:
        fs (float): Fréquence d'échantillonnage en Hz.
        duree (float): Durée du signal généré en secondes.

    Returns:
        tuple: (axe_temporel, signal_genere) sous forme de tableaux numpy.
    """
...
def filtrer_signal(input_signal, sampling_rate, type_filtre, ordre_filtre,
                   frequence_coupure=None, frequence_basse=None,
                   frequence_haute=None):
    """Applique un filtre numérique de Butterworth sur un signal.

    Args:
        input_signal (np.ndarray): Signal numérique d'entrée.
        sampling_rate (float): Fréquence d'échantillonnage en Hz.
        type_filtre (str): "passe_bas", "passe_haut" ou "passe_bande".
        ordre_filtre (int): Ordre du filtre Butterworth.
        frequence_coupure (float, optional): Fréquence de coupure (Hz).
        frequence_basse (float, optional): Fréquence basse du passe-bande (Hz).
        frequence_haute (float, optional): Fréquence haute du passe-bande (Hz).

    Returns:
        np.ndarray: Le signal filtré par la méthode scipy.signal.lfilter.

    Raises:
        ValueError: Si les paramètres de fréquences requis manquent ou
            si le type de filtre est inconnu.
    """
    ...
    if type_filtre == "passe_bas":
        ...
        if frequence_basse is None or frequence_haute is None:
            raise ValueError("Il faut spécifier une fréquence basse et haute.")
...
def valider_frequences(type_filtre, fc, fc2, nyq):
    """Vérifie que les fréquences de coupure respectent le critère de Nyquist.

    Args:
        type_filtre (str): "passe_bas", "passe_haut" ou "passe_bande".
        fc (float): Fréquence de coupure ou fréquence basse (Hz).
        fc2 (float): Fréquence de coupure haute pour le passe-bande (Hz).
        nyq (float): Fréquence de Nyquist (fs / 2) en Hz.

    Raises:
        ValueError: Si une fréquence est négative, dépasse Nyquist, ou si
            l'ordre des bandes est incorrect.
    """
    if fc <= 0:
...
def normalisation_signal(signal):
    ...
    Args:
        signal (np.ndarray): Signal à normaliser.

    Returns:
        np.ndarray: Signal d'amplitude maximale égale à 1 ou -1.
    """
...
def extraction_amp_freq(signal, fs):
    ...
    Détermine la composante fréquentielle principale via une transformée de
    Fourier rapide (FFT).

    Args:
        signal (np.ndarray): Le signal à analyser.
        fs (float): La fréquence d'échantillonnage du signal (Hz).

    Returns:
        dict: Contient "max_amplitude" (float) et "dominant_frequency" (float).
    """
    ...
    return {
        ...
        "dominant_frequency": frequence_dominante
    }

# FONCTIONS D'AFFICHAGE ET RENDER GRAPHES

def mise_a_jour_graph(win):
    """Redessine les deux graphes matplotlib intégrés dans l'interface.

    Affiche sur le graphique supérieur le signal brut et/ou le signal filtré
    selon l'état des cases à cocher. Le graphique inférieur présente le signal
    filtré seul.
    Graphe haut : signal brut (bleu) et signal filtré (rouge) superposés.
    Graphe bas  : signal filtré seul (rouge).
    Args:
        win (MainApp): Instance de la fenêtre de l'application.
    """
    win.ax_haut.clear()
    win.ax_bas.clear()

    # Graphe supérieur : superposition des signaux
    if etat["signal_brut"] is not None and win.check_brut.isChecked():
        win.ax_haut.plot(
            etat["temps"], etat["signal_brut"],
            color="#2563EB", linewidth=0.7, label="Signal brut", alpha=0.8
        )

    if etat["signal_filtre"] is not None and win.check_filtre.isChecked():
        win.ax_haut.plot(
            etat["temps"], etat["signal_filtre"],
            color="#DC2626", linewidth=1.3, label="Signal filtré"
        )
    ...
    # Graphe inférieur : signal filtré unique
...
def description_filtre():
    """Retourne une description lisible textuelle du filtre actif.

    Returns:
        str: Synthèse textuelle du filtre (Type, Ordre, Fréquences).
    """
    m = etat["meta"]
    if m["type"] is None:
        return "—"
...
def affichage_donnees(win):
    """Met à jour les quatre blocs informatifs de widgets de l'interface.

    Actualise les affichages du nom du fichier, du nombre d'échantillons, de
    la fréquence d'échantillonnage et des détails du filtre en cours.
    ...
    # Calcul et affichage de la fréquence dominante si applicable
    if etat["signal_filtre"] is not None:
        try:
            stats = extraction_amp_freq(etat["signal_filtre"], etat["fs"])
            fd = stats["dominant_frequency"]
...
def entete_csv():
    """Génère l'en-tête de métadonnées pour l'exportation au format CSV.

    Returns:
        str: Chaîne de caractères formée de lignes de commentaires `#`.
    """
    m = etat["meta"]
    lignes = [
        "# signal.csv ",
...
def selection_signal(win):
    """Gère l'ouverture du dialogue pour charger un signal à la demande.
    ...
    affichage_donnees(win)
    mise_a_jour_graph(win)
    win.statusBar().showMessage("Signal chargé avec succès")


def declencher_signal_test(win):
    """Génère un signal test par défaut et force la réactualisation graphique.
...
def appliquer_filtre(win):
    """Applique le filtre défini par les réglages actuels de l'IHM.
    ...
    if etat["signal_brut"] is None:
        QMessageBox.warning(
            win, "Aucun signal", "Chargez ou générez d'abord un signal."
        )
        return
...
def reinitialiser(win):
    """Supprime l'historique de filtrage et vide le graphique associé.
...
def save_csv(win):
    """Exporte la feuille de données actuelle (brute/filtrée) au format CSV.
    ...
    if etat["signal_filtre"] is None:
        QMessageBox.warning(
            win, "Aucun signal filtré", "Appliquez d'abord un filtre."
        )
        return
    ...
    QMessageBox.information(
        win, "Export réussi", f"{len(df)} lignes exportées vers :\n{chemin}"
    )
    win.statusBar().showMessage(f"Exporté : {chemin}")


def save_graphique(win):
    """Enregistre une capture du canevas Matplotlib (PNG ou PDF).
    ...
    if etat["signal_brut"] is None:
        QMessageBox.warning(
            win, "Rien à sauvegarder", "Aucun graphe affiché."
        )
        return
...
def connecter_signaux(win):
    """Relie l'ensemble des éléments cliquables IHM aux processus logiques.
    ...
    win.btn_generate.clicked.connect(lambda: declencher_signal_test(win))
    ...
    # Gestion synchrone Slider <-> Formulaire (Fréquence de coupure)
    ...
    # Gestion synchrone Slider <-> Formulaire (Ordre du Filtre)
